...
| **37 — Centralised peer + group registry** | Refactored every peer / device / group lookup into two `Map`-backed registries on `FakeWaServer`: `peerRegistry` (device JID → `FakePeer`) and `groupRegistry` (group JID → `FakeGroupMetadata`). The constructor now registers a SINGLE global IQ handler each for `usync`, `prekey-fetch` and `<iq xmlns="w:g2"><query/></iq>` — they parse the inbound stanza's user/device/group jids and consult the registries instead of relying on per-peer first-match-wins handlers (which was broken for multi-peer scenarios). `createFakePeer` and `createFakePeerWithDevices` (the new clean name for `createFakePeerGroup`) just populate the peer registry. New `createFakeGroup({ groupJid, subject, participants })` lets a test register a group with a fixed participant set so the global w:g2 handler can resolve it. Two pre-existing tests (`client-group-message-decrypt`, `bidirectional-group`) were migrated off their inline `w:g2` handlers and now use `createFakeGroup`. Also fixed a real concurrency bug in `FakePeerGroupRecvSession.decryptGroupMessage`: the lib's `verifySignalSignature` mutates byte 63 of its input signature in place (clears the high bit, restores in `finally`), so when 3 peers verified the same shared `<enc type="skmsg"/>` buffer concurrently the mutations raced and corrupted each other's verifies — fixed by `slice()`-ing the signature into a fresh buffer per call | New `multi-participant-group.cross-check.test.ts`: pairs a real client, creates 3 distinct fake peers under different user JIDs via `createFakePeer`, registers them as participants of one fake group, calls `client.sendMessage(groupJid, conversation)`, asserts the captured stanza carries `<participants>` with 3 `<to>` children (one per peer) plus the top-level `<enc type=skmsg>`, and each peer's `expectGroupMessage` decrypts the same plaintext via its own session | ✅ done |
