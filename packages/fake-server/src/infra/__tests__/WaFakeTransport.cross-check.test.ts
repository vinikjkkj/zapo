...
async function runXxHandshake(): Promise<{
    ...
    server.authenticate(parsedM1.clientHello.ephemeral)

    server.authenticate(serverEphemeral.pubKey)
    server.mixIntoKey(
        await X25519.scalarMult(serverEphemeral.privKey, parsedM1.clientHello.ephemeral)
    )
    const ctServerStatic = server.encrypt(serverStatic.pubKey)
    server.mixIntoKey(await X25519.scalarMult(serverStatic.privKey, parsedM1.clientHello.ephemeral))
    const ctPayload = server.encrypt(new Uint8Array([0x00]))
    ...
    client.decrypt(sh.payload)

    const ctClientStatic = client.encrypt(clientStatic.pubKey)
    client.mixIntoKey(await X25519.scalarMult(clientStatic.privKey, serverEphemeral.pubKey))
    const ctClientPayload = client.encrypt(new Uint8Array([0x00]))
...
test('post-handshake transport: server encrypts, client decrypts (3 frames in a row)', async () => {
    ...
    for (let i = 0; i < messages.length; i += 1) {
        ...
        const decoded = clientSocket.decrypt(ct)
...
test('post-handshake transport: client encrypts, server decrypts (3 frames in a row)', async () => {
    ...
    for (let i = 0; i < messages.length; i += 1) {
        const ct = clientSocket.encrypt(messages[i])
...
test('post-handshake transport: bidirectional interleaved frames stay aligned', async () => {
    ...
    for (let i = 0; i < 4; i += 1) {
        ...
        ctsFromClient.push(clientSocket.encrypt(new Uint8Array([0x20 + i])))
    }

    for (let i = 0; i < 4; i += 1) {
        const fromServer = clientSocket.decrypt(ctsFromServer[i])
