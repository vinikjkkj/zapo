...
async function createSession(): Promise<Harness> {
    ...
    return { session, reactions, broadcast, internals }
}
...
test('an audio call derives and declares its own app-data ssrc', async (t) => {
    ...
    assert.ok(
        internals.selfStreamSsrcs.includes(selfAppData),
        'the app-data ssrc must be declared to the relay'
    )
    assert.ok(internals.peerStreamSsrcs.length >= 2, 'the peer app-data stream must be subscribed')
    assert.ok(
        internals.peerAppDataSsrcs.has(
            generateSecureSsrc(CALL_ID, PEER_JID, WA_SSRC_SLOT.APP_DATA.MAIN)
        )
    )
})
